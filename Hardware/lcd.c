...
void LCD_Showfloat(u16 x, u16 y, float num, u8 len, u8 size)
{
	...
	float xiaoshu=(num-zhengshu)*10;
	if(num<0)
	{
		LCD_ShowString(x-10,y,210,24,16,"-");
		LCD_ShowNum(x,y,-zhengshu,len,size);
		LCD_ShowString(x+30,y,210,24,16,".");
		LCD_ShowNum(x+40,y,xiaoshu,1,size);
	}
	else
	{
		LCD_ShowString(x-10,y,210,24,16,"+");
